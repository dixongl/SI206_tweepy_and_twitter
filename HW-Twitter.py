...
cache_fname = 'hw8_cache_dixongl.json'
...
def twitter_function(search):
    if search in cache_dictionary.keys():
        print('using cache')
        ...
        file.write(json.dumps(cache_dictionary))
        file.close()
        print("fetching")
        return cache_dictionary[search]
...
for item in range(3):
    ...
    results = twitter_function(term)
    d[term] = results
    tweets = results['statuses'][:5]
    for tweet in tweets:
